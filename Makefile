...
help:
	...
	@echo "  example        - Run the MIDI example"
	@echo ""
...
clean-all: clean-pyc clean-build clean-test
	...
	@rm -rf examples/output/ 2>/dev/null || true
	@rm -rf *.mid 2>/dev/null || true
...
dev-install:
	...
	@echo "  make example    - Generate example MIDI files"
...
# Run the MCP server
run:
	@echo "Running Music MCP server..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_music.server; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_music.server; \
	fi

# Run the MIDI example
example:
	@echo "Running MIDI example..."
	@if command -v uv >/dev/null 2>&1; then \
		uv run python examples/generate_midi.py; \
	else \
		python3 examples/generate_midi.py; \
	fi
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-music==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-music==$$version"
...
typecheck:
	...
	@if command -v uv >/dev/null 2>&1; then \
		uv run mypy src/chuk_mcp_music/core src/chuk_mcp_music/compiler --ignore-missing-imports; \
	elif command -v mypy >/dev/null 2>&1; then \
		mypy src/chuk_mcp_music/core src/chuk_mcp_music/compiler --ignore-missing-imports; \
	else \
